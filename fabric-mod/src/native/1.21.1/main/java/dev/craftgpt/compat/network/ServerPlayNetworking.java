...
public final class ServerPlayNetworking {
    ...
    public static <T extends CustomPacketPayload> void registerGlobalReceiver(CustomPacketPayload.Type<T> type, Handler<T> handler) {
        ...
        if (!initialized) {
            net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.registerGlobalReceiver(NativeNetwork.Frame.TYPE,
                (payload, context) -> receive(payload.value(), new Context(context.server(), context.player())));
    ...
    public static boolean canSend(ServerPlayer player, CustomPacketPayload.Type<?> type) {
        return net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.canSend(player, NativeNetwork.Frame.TYPE);
    }
    public static void send(ServerPlayer player, CustomPacketPayload payload) {
        PacketTransport.send(PayloadTypeRegistry.playS2C(), payload,
            frame -> net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.send(player, new NativeNetwork.Frame(frame)));
    }
}
